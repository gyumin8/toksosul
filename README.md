...
- **`GEMINI_TEXT_MODEL`은 `gemini-3.5-flash-lite`를 그대로 쓴다 (2026-09-17 실측 후 결정).**
  `gemini-3.5-flash`(비-lite)로 업그레이드를 시도했다가 되돌렸다. 처음 테스트에선
  flash가 계속 429로 실패했는데, 이건 모델 문제가 아니라 짧은 시간에 스토리 여러 개를
  연속 테스트하면서 백그라운드 후처리(삽화 장면묘사·떡밥추적·완결추천이 각각 별도
  Gemini 호출을 낸다)까지 겹쳐 RPM 한도를 태워버린 테스트 방법 문제였다. 할당량이
  리셋된 직후 `ai.continue_story()`만 다른 호출 없이 단독으로(HTTP/DB/백그라운드 제외)
  같은 시나리오 3개로 다시 비교했더니 flash도 11/11 성공했다 — 그런데도 flash-lite로
  유지하기로 했다:
  - 응답 속도가 평균 12.4초로 flash-lite(4.6초)의 2.7배. 턴제로 서로 기다리는 앱이라
    체감이 크다.
  - 같은 로맨스 시나리오 같은 턴에서 flash가 시점(POV) 혼선을 냈다 — 서연이 1인칭으로
    쓴 턴인데 이어쓰기가 "나"를 규민 쪽으로 바꿔버리고, 카톡(비대면 문자) 상황을
    마치 같은 자리에서 지켜보는 것처럼 묘사했다. flash-lite는 같은 자리에서 시점을
    안 흔들었다.
  - 스릴러 시나리오에서 "피비린내" 표현이 독립된 두 번의 테스트 모두 같은 턴에서
    나왔다 — flash가 긴장 장면을 유독 자극적인(피/폭력) 쪽으로 몰아가는 경향이
    재현성 있게 관찰됐다. 이게 실제로 Cloudflare NSFW 오탐 → Gemini 폴백도 할당량
    부족으로 실패 → 플레이스홀더로 대체되는 연쇄로 이어진 사례가 있었다.
  결론: 문장 표현력 자체는 flash가 조금 더 풍부하지만, 속도·시점 일관성·톤 안정성
  전부 flash-lite가 우세해서 바꿀 이유가 없다고 판단했다. Gemini 외 다른 벤더
  (OpenAI/Anthropic 등)로 교체하는 것도 검토했으나, 이 프로젝트는 무료 등급 운용이
  전제라 카드 없이 계속 쓸 수 있는 무료 한도가 Gemini만큼 넉넉한 대안이 마땅치 않아
  보류했다.
- **배포는 Render + Neon + Supabase Storage 조합으로 간다 (카드 불필요, 기한 없음).**
  Render(앱 서버)·Railway·Fly.io를 비교했는데, Railway는 실제 카드 요구 여부가
  출처마다 엇갈렸고 Fly.io는 조직 생성 자체에 카드가 필수인 데다 무료 체험이
  머신당 5분 제한이라 상시 운영이 안 된다. Render 무료 티어(카드 불필요)로
  정했다 — 15분 무응답 시 절전, 다음 요청 때 약 1분 대기라는 단점이 있지만
  "언제 접속할지 모른다"는 조건에서는 오히려 안 쓰는 동안 비용이 없다는
  장점으로 볼 수 있다.
  다만 Render는 재배포/재시작마다 로컬 디스크를 초기화한다. 이게 두 가지를
  건드린다:
  - **DB**: Render 자체 무료 Postgres는 30일 후 만료된다. 대신 **Neon**을 쓴다
    — 공식 문서에 "영구 무료, 카드 불필요"로 명시돼 있다(0.5GB, 100 CU-h/월).
    5분 유휴 후 컴퓨트가 잠들지만 다음 쿼리가 오면 수백ms 안에 자동으로
    깨어난다 — 사람이 뭘 누를 필요가 없다.
  - **이미지**: 삽화를 `static/media`에 로컬 저장하던 걸 그대로 두면 재시작마다
    사라진다. 처음엔 Cloudflare R2(무료 10GB)를 검토했으나 R2는 **카드 등록이
    필수**라 제외했다. 대신 **Supabase Storage**(무료 1GB, 카드 불필요)에
    올리도록 `ai._save_image()`를 고쳤다. Supabase 자격증명(`SUPABASE_*`)이
    전부 채워졌을 때만 쓰고(`config.USE_SUPABASE_STORAGE`), 하나라도 비어
    있으면 기존처럼 로컬 디스크에 저장한다 — 로컬 개발 중엔 계정 없이도 그대로
    테스트할 수 있어야 하기 때문이다. 업로드는 Supabase가 제공하는 S3 호환
    API로 하는데, 이건 요청마다 AWS SigV4 서명이 필요해서 직접 구현하면 서명
    버그가 나기 쉽다. 다른 Cloudflare 호출은 전부 `urllib`만 쓰지만 여긴
    예외적으로 `boto3`를 추가했다 — 서명 알고리즘이 걸린 문제라 검증된
    라이브러리를 쓰는 쪽이 더 안전하다고 판단했다.
    **주의**: Supabase 무료 프로젝트는 1주일 동안 아무 요청이 없으면 프로젝트
    전체(DB든 Storage든)가 일시정지되고, Neon과 달리 대시보드에서 사람이
    직접 "resume"을 눌러야 풀린다. DB는 Neon을 쓰므로 영향 없지만, 1주일 넘게
    아무도 안 들어오면 그사이 만들어진 이미지가 일시적으로 깨져 보일 수 있다
    — 이때는 Supabase 대시보드에서 프로젝트를 resume하면 된다.
