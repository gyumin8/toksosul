...
- **`GEMINI_TEXT_MODEL`은 `gemini-3.5-flash-lite`를 그대로 쓴다 (2026-09-17 실측 후 결정).**
  ...
  보류했다.
- **배포는 Render + Neon + Cloudflare R2 조합으로 간다 (카드 불필요, 기한 없음).**
  ...
  - **이미지**: 삽화를 `static/media`에 로컬 저장하던 걸 그대로 두면 재시작마다
    사라진다. **Cloudflare R2**(무료 10GB, 카드 불필요, 이미 쓰고 있는 Cloudflare
    계정 그대로 사용)에 올리도록 `ai._save_image()`를 고쳤다. R2 자격증명
    (`CF_R2_*`)이 전부 채워졌을 때만 R2를 쓰고(`config.USE_R2`), 하나라도
    비어 있으면 기존처럼 로컬 디스크에 저장한다 — 로컬 개발 중엔 R2 계정 없이도
    그대로 테스트할 수 있어야 하기 때문이다. R2 업로드는 S3 호환 API로 하는데,
    이건 요청마다 AWS SigV4 서명이 필요해서 직접 구현하면 서명 버그가 나기
    쉽다. 다른 Cloudflare 호출은 전부 `urllib`만 쓰지만 여긴 예외적으로
    `boto3`를 추가했다 — 서명 알고리즘이 걸린 문제라 검증된 라이브러리를
    쓰는 쪽이 더 안전하다고 판단했다.
